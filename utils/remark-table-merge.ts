import { visit } from 'unist-util-visit'

function getText(node: any): string {
  if (!node || !node.children) return ''
  return node.children.map((c: any) => c.value || getText(c)).join('').trim()
}

export default function remarkTableMerge() {
  return (tree: any) => {
    let tableFound = false
    visit(tree, 'table', (tableNode: any) => {
      tableFound = true
      const rows = tableNode.children || []
      
      // grid: 2D array storing references and span information
      const grid = rows.map((row: any) => {
        return (row.children || []).map((cell: any) => ({
          node: cell,
          colSpan: 1,
          rowSpan: 1,
          deleted: false,
          text: getText(cell)
        }))
      })

      // Process merges
      for (let r = 0; r < grid.length; r++) {
        for (let c = 0; c < grid[r].length; c++) {
          const cell = grid[r][c]
          
          if (cell.text === '>') {
            let targetC = c - 1
            // 削除済みのセルをスキップして左の有効な結合先を探す
            while (targetC >= 0 && grid[r][targetC].deleted) {
              targetC--
            }
            if (targetC >= 0) {
              grid[r][targetC].colSpan += cell.colSpan
              cell.deleted = true
            }
          } 
          else if (cell.text === '~') {
            let targetR = r - 1
            // 削除済みのセルをスキップして上の有効な結合先を探す
            while (targetR >= 0 && grid[targetR][c].deleted) {
              targetR--
            }
            if (targetR >= 0) {
              grid[targetR][c].rowSpan += cell.rowSpan
              cell.deleted = true
            }
          }
        }
      }

      // Apply AST modifications
      for (let r = 0; r < rows.length; r++) {
        const rowNode = rows[r]
        // newChildren 配列で削除せず、全セルをそのまま維持する
        
        for (let c = 0; c < grid[r].length; c++) {
          const info = grid[r][c]
          const node = info.node
          node.data = node.data || {}
          node.data.hProperties = node.data.hProperties || {}
          
          if (info.deleted) {
            // physics削除すると mdast-util-to-hast が勝手に空セルを足りない分補完してしまい
            // 逆にテーブルレイアウトが壊れるため、論理的に display: none とする
            node.data.hProperties.style = 'display: none;'
            // 中身のテキストも消しておく
            node.children = []
          } else {
            if (info.colSpan > 1) {
              node.data.hProperties.colspan = info.colSpan
              node.data.hProperties.style = 'text-align: center;'
            }
            if (info.rowSpan > 1) {
              node.data.hProperties.rowspan = info.rowSpan
              node.data.hProperties.style = 'vertical-align: middle;'
            }
          }
        }
      }
    })
  }
}
