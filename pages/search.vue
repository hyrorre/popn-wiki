...
type SearchResult = {
  ...
  revision: number
  updatedAt: string
  matchTypes: Array<'title' | 'path' | 'body' | 'comment'>
  snippet: string
  snippetType: 'title' | 'path' | 'body' | 'comment'
...
const { data, status } = await useAsyncData<SearchResponse>(
  async () => {
    if (!canSearch.value) {
...
const resultLabel = computed(() => {
  ...
  return `「${data.value.query}」の検索結果 ${data.value.total}件`
})
...
<template>
  ...
  <u-container class="py-8">
    ...
    <div
      ...
      class="rounded-2xl border border-dashed border-default bg-gray-50 py-20 text-center dark:bg-gray-800/50"
    >
      <p class="font-medium text-muted">該当するページは見つかりませんでした。</p>
    </div>
      ...
      >
        <div class="flex flex-col gap-3">
          ...
          <div class="flex flex-wrap gap-2">
            ...
            <u-badge v-if="item.commentCountMatched > 0" color="warning" variant="subtle">
              コメント {{ item.commentCountMatched }}件
            </u-badge>
          </div>
