    ...
    <div class="py-8 min-h-[600px]">
      <div class="mb-8 border-b border-default pb-6">
        ...
        <p class="text-muted mt-2">
          あなたのコメントに対する返信や、参加しているスレッドの更新を表示します。
        </p>
      </div>
      ...
      <div v-else-if="replies && replies.length > 0" class="flex flex-col gap-4">
        <u-card v-for="reply in replies" :key="reply.id" class="hover:border-primary/50 transition-all duration-300 shadow-sm hover:shadow-md group">
          <div class="flex items-start gap-4">
            ...
            <div class="flex-1 min-w-0">
              ...
              <div class="mb-3">
                <u-link :to="`/${reply.path}`" class="text-xs font-semibold text-primary hover:underline flex items-center gap-1 max-w-full">
                  <u-icon name="i-heroicons-document-text" class="w-3.5 h-3.5 shrink-0" />
                  <span class="truncate">{{ reply.pageTitle || reply.path }}</span>
                </u-link>
              </div>

              <div class="bg-gray-50 dark:bg-gray-800/50 p-3 rounded-lg border border-default relative group-hover:bg-white dark:group-hover:bg-gray-900 transition-colors">
      ...
      <div v-else class="text-center py-20 bg-gray-50 dark:bg-gray-800/50 rounded-2xl border border-dashed border-default">
        <div class="mb-4 bg-white dark:bg-gray-900 w-20 h-20 rounded-full flex items-center justify-center mx-auto shadow-sm">
